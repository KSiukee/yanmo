...
//! 取证现场不该被自己改动。它不改的从来是稿件内容：正文一个字都不动。
...
/// 体检一份库（给"不丢稿"演练核对用）：把 JSON **写到文件**（默认写在库旁边，`--out` 可换），返回退出码。
///
/// 为什么写文件而不是打印：研墨是 GUI 子系统程序，在 cmd 控制台里标准输出是无效的，
/// `println!` 写了看不见（演练当场踩到）。文件不会骗人，脚本再用 `type` 打出来。
///
/// 它**不是只读**（评审：中等 18）：`Store::open` 会跑结构迁移、登记本机、给老库回填
/// 字数标记，还可能写 `-wal`（SQLite 的正常行为）。稿件内容一个字都不动，但"现场没被
/// 自己改动"这句话不成立——取证 / 体检坏库前先复制一份库文件。
