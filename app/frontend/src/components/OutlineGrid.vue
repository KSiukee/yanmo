...
// 「大纲」这一屏的**外壳**：遮罩 + 标题 + 两片页签（总纲 / 章纲）+ Esc 分层退。
//
// 为什么要有两页（0.68.1 按真机反馈改的）：作者点「大纲」想看的先是"这本书讲什么"
// （总纲），再是"每章怎么落"（章纲那张大表）。总纲原先藏在「资料」弹窗的第一页，
// 等于把最该先看的东西放到第三个入口后面——挪到这儿，打开就在眼前。
//
// 两页各自成件（[`StorylinePane`](./StorylinePane.vue) / [`OutlineTable`](./OutlineTable.vue)）：
// 这个文件只管"现在露哪一页、Esc 该怎么退"，别的什么都不碰。
//
...
/** 两片页签（稳定码 → 字典 `grid.tab.*`）：默认落在总纲。 */
const PANES: GridPane[] = ["storyline", "chapters"];
...
<template>
  <div v-if="visible" class="grid dialog" @click.self="hide()">
    <section class="grid__box dialog__box" @keydown.esc.stop="onEscape()">
      ...
      <!-- 第二片：章纲（那一张大表） -->
      <OutlineTable v-else :session="session" />
    </section>
  </div>
</template>
