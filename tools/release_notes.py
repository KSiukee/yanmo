...
- **`研墨-{version}-setup.exe`** —— **安装版**：开始菜单里能找到，随时可卸载。多数人选这个。
- **`研墨-{version}.exe`** —— **便携版**：双击即用，不写注册表，稿子仍放在系统里的数据位置。
- **`研墨-{version}-绿色版.zip`** —— **绿色版**（解压即用）：稿子放在解压出来的那个文件夹里，
...
给两个入口：「**建一本书**」（一次问清书名、类型、简介、卷章命名），或者「**就在这一本里写**」
（自动建的那本空白书还没名字，起个名就能开写）。之后就只剩写作本身了。
"""
